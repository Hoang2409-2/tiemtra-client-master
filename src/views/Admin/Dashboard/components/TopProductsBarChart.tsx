...
export default function TopProductsBarChart({ data, loading }: TopProductsBarChartProps) {
  // Convert API data to chart format
  const chartData: ProductBar[] = data?.map(item => ({
  ...
  return (
    <Card sx={{ borderRadius: 3, height: 360, boxShadow: "0 8px 30px rgba(0,0,0,0.06)" }}>
      <CardContent sx={{ height: "100%" }}>
        ...
        <Chip size="small" label={`Top ${Math.min(5, chartData.length)}`} sx={{ mb: 1 }} />
        <Divider sx={{ mb: 2 }} />
